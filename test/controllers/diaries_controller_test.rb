...
class DiariesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create diary for user one" do
    sign_in @user_one
    post diaries_path, params: { diary: { diary_date: Date.today, weather: "晴れ", catch_count: "1匹", time_of_day: "朝まづめ", temperature: "氷点下", content: "Test Content" } }
  ...
  test "should create diary for user two" do
    sign_in @user_two
    post diaries_path, params: { diary: { diary_date: Date.today, weather: "曇り", catch_count: "2匹", time_of_day: "夕まづめ", temperature: "0~10℃", content: "Another Test Content" } }
    assert_response :redirect
  end
