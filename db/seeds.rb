User.all.find_each do |user| # すべてのユーザーを取得してループ
  100.times do
    Catch.create!(
      user: user,
  ...
  puts "100 catch records have been created for user with ID: #{user.id}!"
  ...
  100.times do
    Diary.create!(
      ...
      diary_date: Faker::Date.between(from: 2.years.ago, to: Date.today),
      created_at: Faker::Time.between(from: 2.years.ago, to: Time.now),
      updated_at: Faker::Time.between(from: 2.years.ago, to: Time.now)
    )
  end

  puts "100 diary records have been created for user with ID: #{user.id}!"
end
